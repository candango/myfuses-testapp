    ...
    public function authenticate( MyFusesSecurityManager $manager ) {
        ...
        if( $userLogin != "" && $userPassword != "" ) {
            if( !array_key_exists( $userLogin, $users ) ) {
            	$_SESSION[ 'MYFUSES_SECURITY' ][ 'AUTH_ERRORS' ][ 'error' ] = 'Invalid Login!';
            }
            else {
            ...
            if( $userLogin == "" ) {
                $_SESSION[ 'MYFUSES_SECURITY' ][ 'AUTH_ERRORS' ][ $manager->getUserLoginField() ] = 
                    "The user name is a required Field.";
            }
            if( $userPassword == "" ) {
                $_SESSION[ 'MYFUSES_SECURITY' ][ 'AUTH_ERRORS' ][ $manager->getUserPasswordField() ] = 
